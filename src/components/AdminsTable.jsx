...
const AdminsTable = () => {
    ...
    const fetchAdministratori = async () => {
        try {
            ...
            setAdministratori(response.data);
        } catch (err) {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            if (modalMode === 'create') {
                ...
                await api.put(`/api/administratori/${formData.id}`, payload);
            }
    ...
    const confirmDelete = async () => {
        try {
            await api.delete(`/api/administratori/${deleteAlert.id}`);
            setDeleteAlert({ isOpen: false, id: null });
            fetchAdministratori();
        } catch (err) {
            console.error("Greška pri brisanju:", err);
            alert('Došlo je do greške prilikom brisanja administratora.');
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        return data;
    }, [administratori, filters, sortConfig]);
    ...
    if (loading) return <div className="p-4 text-slate-500">Učitavanje administratora...</div>;
    if (error) return <div className="p-4 text-red-500 font-medium">{error}</div>;

    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-white flex justify-between items-center">
                <h3 className="text-lg font-bold text-slate-800">Popis svih administratora</h3>
                <button
                    onClick={openCreateModal}
                    className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors shadow-sm cursor-pointer"
                >
                        ...
                        <tr className="bg-slate-100 border-b border-slate-200">
                            {[
                                ...
                                <th key={col.key} className="p-3 font-semibold text-slate-700 text-sm align-top min-w-[150px]">
                                    <div
                                        className="flex items-center gap-1 cursor-pointer hover:text-indigo-600 mb-2 transition-colors select-none"
                                    ...
                                    <select
                                        ...
                                        className="w-full px-2 py-1.5 text-xs border border-slate-300 rounded focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 font-normal shadow-sm bg-white text-slate-700 cursor-pointer"
                                    >
                        ...
                        {processedData.length > 0 ? (
                            processedData.map((item) => (
                                <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-3 text-sm text-indigo-600">{item.email}</td>
                                    <td className="p-3 text-sm text-slate-500">{formatDate(item.kreiran_na)}</td>
                                    <td className="p-3 text-sm text-center">
                                        <div className="flex justify-center gap-2">
                                            <button onClick={() => openEditModal(item)} className="text-indigo-600 hover:text-indigo-800 font-medium cursor-pointer p-1" title="Uredi">✎</button>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" required value={formData.ime} onChange={(e) => setFormData({ ...formData, ime: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1">Prezime <span className="text-red-500">*</span></label>
                                    <input type="text" required value={formData.prezime} onChange={(e) => setFormData({ ...formData, prezime: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-1">Email adresa <span className="text-red-500">*</span></label>
                                <input type="email" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                            </div>
                            ...
                            <div>
                                ...
                                <input type="password" required={modalMode === 'create'} value={formData.lozinka} onChange={(e) => setFormData({ ...formData, lozinka: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                            </div>
                            ...
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                <button type="submit" className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition-colors cursor-pointer">
                                    {modalMode === 'create' ? 'Spremi administratora' : 'Spremi izmjene'}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            {deleteAlert.isOpen && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-sm text-center">
                        ...
                        <h2 className="text-xl font-bold text-slate-800 mb-2">Brisanje administratora</h2>
                        <p className="text-slate-500 mb-6">Jeste li sigurni da želite obrisati ovog administratora? Ova akcija se ne može poništiti.</p>
